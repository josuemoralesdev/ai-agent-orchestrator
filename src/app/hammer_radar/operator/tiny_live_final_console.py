...
def build_autonomous_armed_dry_run_panel(
    *,
    log_dir: str | Path | None = None,
    autonomous_arming_config_path: str | Path | None = None,
) -> dict[str, Any]:
    packet = build_tiny_live_autonomous_armed_dry_run(
        log_dir=log_dir,
        config_path=autonomous_arming_config_path,
    )
...
def build_autonomous_trigger_scheduler_activation_panel() -> dict[str, Any]:
    packet = build_autonomous_trigger_scheduler_activation_readiness()
    panel = packet.get("autonomous_trigger_scheduler_activation_panel")
    if isinstance(panel, Mapping):
        panel = dict(panel)
    else:
        panel = {}
    panel.update(
        {
            "activation_readiness_status": panel.get("activation_readiness_status")
            or packet.get("status"),
            "activation_checklist_path": packet.get("activation_checklist_path"),
            "activation_print_only_script_path": packet.get("activation_print_only_script_path"),
            "next_manual_operator_step": panel.get("next_manual_operator_step")
            or packet.get("next_manual_operator_step"),
...
def build_test_only_matching_candidate_trigger_certificate_panel(
    *,
    log_dir: str | Path | None = None,
) -> dict[str, Any]:
    packet = build_status_tiny_live_test_only_matching_candidate_trigger_certificate(log_dir=log_dir)
    panel = packet.get("test_only_matching_candidate_trigger_certificate_panel")
    if isinstance(panel, Mapping):
        panel = dict(panel)
    else:
        panel = {}
    panel.update(
        {
            "status": panel.get("status") or packet.get("status"),
            "requested_lane_key": panel.get("requested_lane_key") or packet.get("requested_lane_key"),
            "simulation_flag_required": panel.get("simulation_flag_required")
            if "simulation_flag_required" in panel
            else not (
                packet.get("simulate_matching_fresh_candidate_for_tests_only") is True
                or packet.get("simulate_nonmatching_fresh_candidate_for_tests_only") is True
            ),
            "timer_observed_wait_certificate_summary": panel.get("timer_observed_wait_certificate_summary")
            or {
                "status": packet.get("timer_observed_wait_certificate_status"),
                "timer_health_status": packet.get("timer_health_status"),
                "timer_active": packet.get("timer_active") is True,
                "recent_tick_seen": packet.get("recent_tick_seen") is True,
                "recent_tick_count": int(packet.get("recent_tick_count") or 0),
            ...
            or {
                ...
                "candidate_matches_requested_lane": packet.get("candidate_matches_requested_lane") is True,
                "exact_lane_only": True,
                "no_cross_lane_borrowing": True,
                "simulated_candidate_source": packet.get("simulated_candidate_source"),
                "simulated_candidate_not_real_market_data": (
                    packet.get("simulated_candidate_not_real_market_data") is True
                ),
            },
...
def build_real_candidate_dry_run_trigger_bridge_panel(
    *,
    log_dir: str | Path | None = None,
) -> dict[str, Any]:
    packet = build_status_tiny_live_real_candidate_dry_run_trigger_bridge(log_dir=log_dir)
    panel = packet.get("real_candidate_dry_run_trigger_bridge_panel")
    if isinstance(panel, Mapping):
        panel = dict(panel)
    else:
        panel = {}
    panel.update(
        {
            ...
            "match_status": panel.get("match_status")
            or {
            ...
            "dry_run_simulated_lifecycle_summary": panel.get(
...
def build_manual_operator_dry_run_arming_post_arm_certificate_panel(
    *,
    log_dir: str | Path | None = None,
) -> dict[str, Any]:
    packet = build_status_tiny_live_manual_operator_dry_run_arming_post_arm_certificate(
        log_dir=log_dir
    )
    panel = packet.get("manual_operator_dry_run_arming_post_arm_certificate_panel")
    if isinstance(panel, Mapping):
        panel = dict(panel)
    else:
        panel = {}
    panel.update(
        {
            ...
            "requested_lane_key": panel.get("requested_lane_key")
            or packet.get("requested_lane_key"),
            "manual_operator_arming_observed": panel.get(
                "manual_operator_arming_observed"
            )
            if "manual_operator_arming_observed" in panel
            else packet.get("manual_operator_arming_observed") is True,
            "current_arming_state": panel.get("current_arming_state")
            or {
                "global_auto_live_enabled": packet.get("global_auto_live_enabled")
                is True,
                ...
                "any_lane_auto_armed": packet.get("any_lane_auto_armed") is True,
                "armed_lane_key": packet.get("armed_lane_key"),
                "allowed_lane_keys": list(
                    packet.get("allowed_lane_keys_from_arming_state") or []
                ),
                "lane_auto_live_enabled_keys": list(
                    packet.get("lane_auto_live_enabled_keys") or []
                ),
            },
            "r300_summary": panel.get("r300_summary")
            or {
                "status": packet.get("r300_bridge_status"),
                "exact_lane_auto_armed": packet.get("exact_lane_auto_armed") is True,
                "any_lane_auto_armed": packet.get("any_lane_auto_armed") is True,
                "armed_lane_key": packet.get("armed_lane_key"),
            },
            ...
            "timer_scheduler_summary": panel.get("timer_scheduler_summary")
            or {
                "timer_health_status": packet.get("timer_health_status"),
                "timer_active": packet.get("timer_active") is True,
                "recent_tick_seen": packet.get("recent_tick_seen") is True,
            ...
            or {
                ...
                "post_arm_verification_command": packet.get(
                    "post_arm_verification_command"
                ),
            },
            "post_arm_verification_command": panel.get("post_arm_verification_command")
            or packet.get("post_arm_verification_command"),
            "blockers": list(panel.get("blockers") or packet.get("blockers") or []),
            "recommended_next_operator_move": panel.get("recommended_next_operator_move")
            or packet.get("recommended_next_operator_move"),
            "final_command_available": False,
...
def build_armed_dry_run_timer_observation_certificate_panel(
    *,
    log_dir: str | Path | None = None,
) -> dict[str, Any]:
    packet = build_status_tiny_live_armed_dry_run_timer_observation_certificate(
        log_dir=log_dir
    )
    panel = packet.get("armed_dry_run_timer_observation_certificate_panel")
    if isinstance(panel, Mapping):
        panel = dict(panel)
    else:
        panel = {}
    panel.update(
        {
            ...
            "r301_summary": panel.get("r301_summary")
            or {
                "status": packet.get("r301_post_arm_status"),
                "exact_lane_auto_armed": packet.get("exact_lane_auto_armed") is True,
                "any_lane_auto_armed": packet.get("any_lane_auto_armed") is True,
                "armed_lane_key": packet.get("armed_lane_key"),
            },
            ...
            "blockers": list(panel.get("blockers") or packet.get("blockers") or []),
            "manual_operator_disarm_command": panel.get("manual_operator_disarm_command")
            or packet.get("manual_operator_disarm_command"),
            "recommended_next_operator_move": panel.get("recommended_next_operator_move")
            or packet.get("recommended_next_operator_move"),
            "final_command_available": False,
            "submit_allowed": False,

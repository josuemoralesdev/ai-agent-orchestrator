...
Append normalized `betrayal_source_emitter_v2` rows only when R223 reports `resolver_ready_rows > 0`, R226 reports `resolver_ready_preview_rows > 0`, and R227 reports `resolver_ready_preview_rows > 0`.
...
R224 must explicitly refuse to append normalized rows when R224A evidence collection, R226 renormalization, or R227 direction completion reports zero resolver-ready preview rows, even if contextual evidence exists. Partial evidence is not append authority.

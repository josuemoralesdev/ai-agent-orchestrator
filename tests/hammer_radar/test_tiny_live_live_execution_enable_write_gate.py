...
from src.app.hammer_radar.operator.tiny_live_live_execution_enable_write_gate import (
    ...
    LEDGER_FILENAME,
    TINY_LIVE_LIVE_EXECUTION_ENABLE_WRITE_GATE_READY,
    TINY_LIVE_LIVE_EXECUTION_ENABLE_WRITE_GATE_REJECTED,
    TINY_LIVE_LIVE_EXECUTION_ENABLE_WRITE_GATE_WRITTEN,
    build_live_execution_enable_object,
...
def test_preview_writes_no_execution_enable_artifact(tmp_path: Path) -> None:
    log_dir = _fixture_logs(tmp_path)
    payload = build_tiny_live_live_execution_enable_write_gate(
        log_dir=log_dir,
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        now=NOW,
    )

    assert payload["status"] == TINY_LIVE_LIVE_EXECUTION_ENABLE_WRITE_GATE_READY
...
def test_wrong_confirmation_rejects_and_writes_no_artifact(tmp_path: Path) -> None:
    ...
    payload = build_tiny_live_live_execution_enable_write_gate(
        log_dir=log_dir,
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        write_execution_enable=True,
        confirm_tiny_live_live_execution_enable_write="wrong",
        now=NOW,
    )

    assert payload["status"] == TINY_LIVE_LIVE_EXECUTION_ENABLE_WRITE_GATE_REJECTED
    assert payload["confirmation_valid"] is False
    assert payload["execution_enable_written"] is False
    assert payload["execution_enable_write_overall_status"] == "TINY_LIVE_EXECUTION_ENABLE_WRITE_REJECTED_BAD_CONFIRMATION"
    assert not (log_dir / LEDGER_FILENAME).exists()


def test_exact_confirmation_writes_only_execution_enable_artifact(tmp_path: Path) -> None:
    ...
    records = load_tiny_live_live_execution_enable_write_gate_records(log_dir=log_dir, limit=0)
    assert payload["status"] == TINY_LIVE_LIVE_EXECUTION_ENABLE_WRITE_GATE_WRITTEN
    assert payload["execution_enable_written"] is True
    assert len(records) == 1
    assert records[0]["event_type"] == "TINY_LIVE_LIVE_EXECUTION_ENABLE_WRITE_GATE"
    assert records[0]["execution_enable_written"] is True
    artifact = records[0]["execution_enable"]
    assert artifact["live_authorized"] is True
    assert artifact["live_execution_enabled"] is True
    assert artifact["lane_armed"] is False
    assert artifact["order_payload_allowed"] is False
    assert artifact["binance_call_allowed"] is False
    assert payload["post_write_verification"]["matching_execution_enable_found"] is True
    assert payload["post_write_verification"]["matching_execution_enable_valid"] is True
    assert payload["post_write_verification"]["live_authorized"] is True
    assert payload["post_write_verification"]["live_execution_enabled"] is True
    ...
    assert payload["post_write_verification"]["order_payload_allowed"] is False
    assert payload["post_write_verification"]["binance_call_allowed"] is False


def test_execution_enable_object_validates() -> None:
    artifact = build_live_execution_enable_object(
        latest_r233={"execution_enable_preview_record_id": "r233_fixture"},
...
def test_safety_flags_and_matrix_preserve_non_actions(tmp_path: Path) -> None:
    payload = build_tiny_live_live_execution_enable_write_gate(
        log_dir=_fixture_logs(tmp_path),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        now=NOW,
    )
    matrix = payload["live_execution_enable_write_gate_matrix"]

    assert matrix["r233_preview_ready"] is True
    assert matrix["risk_contract_config_ready"] is True
    assert matrix["authorization_valid"] is True
    assert matrix["execution_enable_valid"] is True
    assert matrix["live_execution_enable_written"] is False
    ...
    assert payload["safety"]["execution_enable_write_gate_only"] is True
...
def test_cli_exists(tmp_path: Path) -> None:
    ...
    assert payload["execution_enable_written"] is False
    assert "tiny-live-live-execution-enable-write-gate" in help_result.stdout
...
def _fixture_logs(tmp_path: Path) -> Path:
    ...
    _append(log_dir / "tiny_live_live_authorization_write_gate.ndjson", _r232_authorization_record())
...
def _r228_packet_record() -> dict[str, object]:
    return {
        ...
        "capture_threshold_recheck": {"official_lane_key": OFFICIAL, "evidence_threshold_ready": True},
...
def _r232_authorization_record() -> dict[str, object]:
    return {
...
def _r233_preview_record() -> dict[str, object]:
    return {
        ...
        "live_execution_enable_requirement_preview": {
            "execution_enable_preview_id": "r233_live_execution_enable_preview_inner_fixture",
            "authorization_reference": "r232_authorization_BTCUSDT_8m_short_ladder_close_50_618",
        ...
        "live_execution_enable_preview_overall_status": "TINY_LIVE_LIVE_EXECUTION_ENABLE_PREVIEW_READY_FOR_FUTURE_GATE",
    }

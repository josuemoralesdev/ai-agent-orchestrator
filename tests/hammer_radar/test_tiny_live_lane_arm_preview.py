...
from src.app.hammer_radar.operator.tiny_live_lane_arm_preview import (
    ...
    LEDGER_FILENAME,
    TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_AUTHORIZATION,
    TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_EVIDENCE,
    TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_EXECUTION_ENABLE,
    TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_RISK_CONTRACT,
    TINY_LIVE_LANE_ARM_PREVIEW_READY,
    TINY_LIVE_LANE_ARM_PREVIEW_READY_FOR_FUTURE_GATE,
    TINY_LIVE_LANE_ARM_PREVIEW_RECORDED,
    TINY_LIVE_LANE_ARM_PREVIEW_REJECTED,
...
def test_preview_writes_no_record(tmp_path: Path) -> None:
    ...
    payload = build_tiny_live_lane_arm_preview(
        log_dir=log_dir,
    ...
    assert payload["status"] == TINY_LIVE_LANE_ARM_PREVIEW_READY
    assert payload["lane_arm_preview_recorded"] is False
    assert payload["lane_arm_preview_record_id"] is None
    assert payload["record_lane_arm_preview_requested"] is False
...
def test_wrong_confirmation_rejects(tmp_path: Path) -> None:
    ...
    payload = build_tiny_live_lane_arm_preview(
        ...
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        lane_controls_path=_write_lane_controls(tmp_path / "lane_controls.json"),
        record_lane_arm_preview=True,
        confirm_tiny_live_lane_arm_preview="wrong",
        now=NOW,
    )

    assert payload["status"] == TINY_LIVE_LANE_ARM_PREVIEW_REJECTED
    assert payload["confirmation_valid"] is False
    assert payload["lane_arm_preview_recorded"] is False
    assert load_tiny_live_lane_arm_preview_records(log_dir=log_dir, limit=0) == []
...
def test_correct_confirmation_records_preview_only(tmp_path: Path) -> None:
    ...
    with (
        ...
        payload = build_tiny_live_lane_arm_preview(
            ...
            record_lane_arm_preview=True,
            confirm_tiny_live_lane_arm_preview=CONFIRM_TINY_LIVE_LANE_ARM_PREVIEW_RECORDING_PHRASE,
            now=NOW,
        )
    ...
    records = load_tiny_live_lane_arm_preview_records(log_dir=log_dir, limit=0)
    assert payload["status"] == TINY_LIVE_LANE_ARM_PREVIEW_RECORDED
    assert payload["lane_arm_preview_recorded"] is True
    assert len(records) == 1
    assert records[0]["event_type"] == "TINY_LIVE_LANE_ARM_PREVIEW"
    assert records[0]["lane_arm_preview_recorded"] is True
    assert records[0]["safety"]["lane_arm_preview_only"] is True
    assert records[0]["safety"]["lane_arm_written"] is False
    assert records[0]["safety"]["lane_armed"] is False
...
def test_preview_requires_r234_execution_enable_artifact(tmp_path: Path) -> None:
    ...
    assert payload["lane_arm_gate_matrix"]["lane_arm_preview_ready"] is False
    assert payload["lane_arm_preview_overall_status"] == TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_EXECUTION_ENABLE
...
def test_preview_requires_r232_authorization_artifact(tmp_path: Path) -> None:
    ...
    assert payload["lane_arm_gate_matrix"]["lane_arm_preview_ready"] is False
    assert payload["lane_arm_preview_overall_status"] == TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_AUTHORIZATION
...
def test_preview_requires_valid_risk_contract_config(tmp_path: Path) -> None:
    ...
    assert payload["lane_arm_gate_matrix"]["lane_arm_preview_ready"] is False
    assert payload["lane_arm_preview_overall_status"] == TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_RISK_CONTRACT
...
def test_preview_requires_evidence_ready(tmp_path: Path) -> None:
    ...
    assert payload["lane_arm_gate_matrix"]["lane_arm_preview_ready"] is False
    assert payload["lane_arm_preview_overall_status"] == TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_EVIDENCE


def test_preview_requires_live_authorized_true(tmp_path: Path) -> None:
    payload = build_tiny_live_lane_arm_preview(
...
def test_preview_requires_live_execution_enabled_true_in_r234_artifact(tmp_path: Path) -> None:
    payload = build_tiny_live_lane_arm_preview(
        log_dir=_fixture_logs(tmp_path, live_execution_enabled=False),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        lane_controls_path=_write_lane_controls(tmp_path / "lane_controls.json"),
        now=NOW,
    )

    assert payload["input_summary"]["live_execution_enabled"] is False
    assert payload["lane_arm_gate_matrix"]["lane_arm_preview_ready"] is False
    assert payload["lane_arm_preview_overall_status"] == TINY_LIVE_LANE_ARM_PREVIEW_BLOCKED_BY_EXECUTION_ENABLE


def test_preview_keeps_lane_order_payload_and_official_lane_unchanged(tmp_path: Path) -> None:
    payload = build_tiny_live_lane_arm_preview(
        log_dir=_fixture_logs(tmp_path),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        lane_controls_path=_write_lane_controls(tmp_path / "lane_controls.json"),
    ...
    matrix = payload["lane_arm_gate_matrix"]

    assert matrix["lane_arm_preview_ready"] is True
    ...
    assert payload["lane_controls_readonly_summary"]["official_lane_already_armed"] is False
    ...
    assert payload["lane_arm_preview_overall_status"] == TINY_LIVE_LANE_ARM_PREVIEW_READY_FOR_FUTURE_GATE
...
def test_preview_does_not_write_configs_env_or_call_network(tmp_path: Path) -> None:
    ...
    assert payload["safety"]["lane_arm_preview_only"] is True
...
def test_cli_exists(tmp_path: Path) -> None:
    ...
    assert payload["target_scope"]["official_lane_key"] == OFFICIAL
    assert payload["lane_arm_preview_recorded"] is False
    assert "tiny-live-lane-arm-preview" in help_result.stdout
...
def _fixture_logs(
    ...
    *,
    ...
    _append(log_dir / "tiny_live_10_of_10_ready_packet.ndjson", _r228_packet_record(evidence_ready=evidence_ready))
    ...
    _append(log_dir / "tiny_live_live_execution_enable_preview.ndjson", _r233_preview_record())
    if include_r234:
        _append(
            log_dir / "tiny_live_live_execution_enable_write_gate.ndjson",
            _r234_execution_enable_record(
...
def _r228_packet_record(*, evidence_ready: bool = True) -> dict[str, object]:
    return {
        ...
        "capture_threshold_recheck": {"official_lane_key": OFFICIAL, "evidence_threshold_ready": evidence_ready},
...
def _r233_preview_record() -> dict[str, object]:
    return {
        "event_type": "TINY_LIVE_LIVE_EXECUTION_ENABLE_PREVIEW",
        "status": "TINY_LIVE_LIVE_EXECUTION_ENABLE_PREVIEW_RECORDED",
        "execution_enable_preview_record_id": "r233_live_execution_enable_preview_fixture",
        "generated_at": NOW.isoformat(),
        "target_scope": {"official_lane_key": OFFICIAL, "live_authorized": True, "live_execution_enabled": False},
        "live_execution_enable_gate_matrix": {
            ...
            "live_execution_enable_preview_ready": True,
            "live_execution_enabled": False,
            "lane_armed": False,
            "order_ready": False,
...
def _r234_execution_enable_record(*, live_authorized: bool = True, live_execution_enabled: bool = True) -> dict[str, object]:
    return {
        ...
        "target_scope": {"official_lane_key": OFFICIAL, "live_authorized": live_authorized},
        "execution_enable": {
            ...
            "direction": "short",
            "entry_mode": "ladder_close_50_618",
            "source_execution_enable_preview_id": "r233_live_execution_enable_preview_fixture",

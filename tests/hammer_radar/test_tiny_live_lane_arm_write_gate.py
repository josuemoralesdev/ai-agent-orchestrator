...
from src.app.hammer_radar.operator.tiny_live_lane_arm_write_gate import (
    ...
    LEDGER_FILENAME,
    TINY_LIVE_LANE_ARM_WRITE_GATE_READY,
    TINY_LIVE_LANE_ARM_WRITE_GATE_REJECTED,
    TINY_LIVE_LANE_ARM_WRITE_GATE_WRITTEN,
    build_lane_arm_object,
...
def test_preview_writes_no_lane_arm_artifact(tmp_path: Path) -> None:
    log_dir = _fixture_logs(tmp_path)
    payload = build_tiny_live_lane_arm_write_gate(
        log_dir=log_dir,
    ...
    assert payload["status"] == TINY_LIVE_LANE_ARM_WRITE_GATE_READY
    assert payload["lane_arm_written"] is False
    assert payload["write_lane_arm_requested"] is False
    assert payload["confirmation_valid"] is False
    assert payload["lane_arm_write_preview"]["would_write"] is True
    assert payload["lane_arm_write_preview"]["lane_arm_artifact"] == "ledger_only"
    assert payload["lane_arm_write_preview"]["lane_controls_would_mutate"] is False
...
def test_wrong_confirmation_rejects_and_writes_no_artifact(tmp_path: Path) -> None:
    ...
    payload = build_tiny_live_lane_arm_write_gate(
        ...
        lane_controls_path=_write_lane_controls(tmp_path / "lane_controls.json"),
        write_lane_arm=True,
        confirm_tiny_live_lane_arm_write="wrong",
        now=NOW,
    )

    assert payload["status"] == TINY_LIVE_LANE_ARM_WRITE_GATE_REJECTED
    assert payload["confirmation_valid"] is False
    assert payload["lane_arm_written"] is False
    assert payload["lane_arm_write_overall_status"] == "TINY_LIVE_LANE_ARM_WRITE_REJECTED_BAD_CONFIRMATION"
    assert not (log_dir / LEDGER_FILENAME).exists()


def test_exact_confirmation_writes_only_lane_arm_artifact(tmp_path: Path) -> None:
    ...
    with (
        ...
        payload = build_tiny_live_lane_arm_write_gate(
            ...
            write_lane_arm=True,
            confirm_tiny_live_lane_arm_write=CONFIRM_TINY_LIVE_LANE_ARM_WRITE_PHRASE,
            now=NOW,
        )
    ...
    records = load_tiny_live_lane_arm_write_gate_records(log_dir=log_dir, limit=0)
    assert payload["status"] == TINY_LIVE_LANE_ARM_WRITE_GATE_WRITTEN
    assert payload["lane_arm_written"] is True
    assert len(records) == 1
    assert records[0]["event_type"] == "TINY_LIVE_LANE_ARM_WRITE_GATE"
    artifact = records[0]["lane_arm"]
    ...
    assert payload["post_write_verification"]["matching_lane_arm_found"] is True
    assert payload["post_write_verification"]["matching_lane_arm_valid"] is True
    assert payload["post_write_verification"]["lane_armed"] is True
    ...
    assert payload["post_write_verification"]["kill_switch_disabled"] is False
    assert payload["lane_arm_write_gate_matrix"]["order_ready"] is False
    assert payload["lane_arm_write_gate_matrix"]["live_ready_today"] is False
    assert payload["operator_lane_arm_write_review_packet"]["operator_should_place_order"] is False


def test_lane_arm_object_validates() -> None:
    artifact = build_lane_arm_object(
        latest_r235={"lane_arm_preview_record_id": "r235_fixture"},
...
def test_safety_flags_preserve_non_actions(tmp_path: Path) -> None:
    ...
    matrix = payload["lane_arm_write_gate_matrix"]

    assert matrix["r235_preview_ready"] is True
    ...
    assert matrix["lane_arm_valid"] is True
    assert matrix["lane_arm_written"] is False
    ...
    assert payload["safety"]["lane_arm_write_gate_only"] is True
...
def test_cli_exists(tmp_path: Path) -> None:
    ...
    assert payload["lane_arm_written"] is False
    assert "tiny-live-lane-arm-write-gate" in help_result.stdout
...
def _fixture_logs(tmp_path: Path) -> Path:
    ...
    _append(log_dir / "tiny_live_live_execution_enable_write_gate.ndjson", _r234_execution_enable_record())
...
def _r234_execution_enable_record() -> dict[str, object]:
    return {
...
def _r235_preview_record() -> dict[str, object]:
    return {
        ...
        "lane_arm_requirement_preview": {"lane_arm_preview_id": "r235_lane_arm_preview_inner_fixture"},

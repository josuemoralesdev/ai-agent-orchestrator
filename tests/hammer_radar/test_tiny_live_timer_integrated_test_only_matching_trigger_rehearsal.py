...
def test_plain_cli_style_without_simulation_blocks(tmp_path: Path) -> None:
    payload = _build(tmp_path)

    assert payload["status"] == TIMER_INTEGRATED_TEST_ONLY_MATCHING_TRIGGER_REHEARSAL_BLOCKED
    assert "missing_test_only_simulation_flag" in payload["blockers"]
    assert payload["simulated_trigger_recorded"] is False
    assert payload["scheduler_rehearsal_iterations_completed"] == 0
    _assert_systemd_defaults(payload)
    _assert_no_submit_or_order(payload)


def test_both_simulation_flags_block(tmp_path: Path) -> None:
    payload = _build(
        tmp_path,
...
def test_matching_simulation_certifies(tmp_path: Path) -> None:
    payload = _build(
        tmp_path,
        simulate_matching_fresh_candidate_for_tests_only=True,
        iterations=1,
    ...
    assert payload["status"] == TIMER_INTEGRATED_TEST_ONLY_MATCHING_TRIGGER_REHEARSAL_CERTIFIED
    ...
    assert payload["bounded_manual_rehearsal_only"] is True
    assert payload["scheduler_integration_rehearsal_supported"] is True
    assert payload["scheduler_rehearsal_iterations_requested"] == 1
    assert payload["scheduler_rehearsal_iterations_completed"] == 1
    assert payload["test_only_matching_certificate_status"] == "TEST_ONLY_MATCHING_CANDIDATE_TRIGGER_CERTIFIED"
    assert payload["timer_observed_wait_certificate_status"] == "TIMER_OBSERVED_ARMED_LANE_WAIT_CERTIFIED"
...
def test_nonmatching_simulation_no_triggers(tmp_path: Path) -> None:
    payload = _build(
        tmp_path,
        simulate_nonmatching_fresh_candidate_for_tests_only=True,
    )

    assert payload["status"] == TIMER_INTEGRATED_TEST_ONLY_NONMATCHING_TRIGGER_REHEARSAL_NO_TRIGGER
    assert payload["simulated_candidate_lane_key"] != LANE_44M_LONG
    assert payload["candidate_matches_requested_lane"] is False
    assert payload["simulated_trigger_recorded"] is False
    assert payload["simulated_lifecycle_status"] == "SIMULATED_DRY_RUN_LIFECYCLE_NOT_RECORDED"
    _assert_no_submit_or_order(payload)


def test_bounded_iterations_max_2(tmp_path: Path) -> None:
    payload = _build(
        tmp_path,
        simulate_matching_fresh_candidate_for_tests_only=True,
        iterations=3,
    ...
    assert payload["status"] == TIMER_INTEGRATED_TEST_ONLY_MATCHING_TRIGGER_REHEARSAL_BLOCKED
    assert "iterations_max_2" in payload["blockers"]
    assert payload["scheduler_rehearsal_iterations_requested"] == 3
    assert payload["scheduler_rehearsal_iterations_completed"] == 0
    _assert_no_submit_or_order(payload)


def test_invalid_near_miss_paper_and_non_btc_lanes_block(tmp_path: Path) -> None:
    cases = [
        ("", "lane_key_required"),
...
def test_record_appends_only_r297_ledger(tmp_path: Path) -> None:
    payload = _build(
        tmp_path,
        simulate_matching_fresh_candidate_for_tests_only=True,
        record_timer_integrated_test_only_matching_trigger_rehearsal=True,
    )

    assert payload["timer_integrated_test_only_matching_trigger_rehearsal_recorded"] is True
    assert (tmp_path / "tiny_live_timer_integrated_test_only_matching_trigger_rehearsal.ndjson").exists()
...
def test_api_plain_status_blocks_and_records_nothing(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv(LOG_DIR_ENV_VAR, str(tmp_path))

    response = TestClient(app).get(
        "/tiny-live/timer-integrated-test-only-matching-trigger-rehearsal/status"
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["event_type"] == "TINY_LIVE_TIMER_INTEGRATED_TEST_ONLY_MATCHING_TRIGGER_REHEARSAL"
    assert payload["status"] == TIMER_INTEGRATED_TEST_ONLY_MATCHING_TRIGGER_REHEARSAL_BLOCKED
    assert "missing_test_only_simulation_flag" in payload["blockers"]
    assert payload["simulation_flag_required"] is True
    assert payload["simulated_trigger_recorded"] is False
    assert payload["record_timer_integrated_test_only_matching_trigger_rehearsal_requested"] is False
    assert payload["timer_integrated_test_only_matching_trigger_rehearsal_recorded"] is False
    assert not (tmp_path / "tiny_live_timer_integrated_test_only_matching_trigger_rehearsal.ndjson").exists()
    _assert_no_submit_or_order(payload)


def test_api_explicit_simulation_certifies_and_records_nothing(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv(LOG_DIR_ENV_VAR, str(tmp_path))
    monkeypatch.setattr(
        ...
        lambda **kwargs: _build(
            tmp_path,
            lane_key=kwargs.get("lane_key") or LANE_44M_LONG,
            simulate_matching_fresh_candidate_for_tests_only=kwargs.get(
                "simulate_matching_fresh_candidate_for_tests_only", False
            ),
            simulate_nonmatching_fresh_candidate_for_tests_only=kwargs.get(
                "simulate_nonmatching_fresh_candidate_for_tests_only", False
            ),
...
def test_final_console_panel_safe_by_default(tmp_path: Path) -> None:
    ...
    panel = payload["timer_integrated_test_only_matching_trigger_rehearsal_panel"]

    assert panel["status"] == TIMER_INTEGRATED_TEST_ONLY_MATCHING_TRIGGER_REHEARSAL_BLOCKED
    assert panel["simulation_flag_required"] is True
    assert "missing_test_only_simulation_flag" in panel["blockers"]
    assert panel["installed_systemd_timer_mutated"] is False
    assert panel["installed_systemd_timer_fake_candidate_injection_enabled"] is False
    assert panel["normal_scheduler_default_simulation_enabled"] is False
...
def test_cli_help_exposes_r297_flags() -> None:
    ...
    assert "--record-timer-integrated-test-only-matching-trigger-rehearsal" in result.stdout


def test_print_only_script_contains_no_executable_dangerous_command_behavior() -> None:
    script = REPO_ROOT / "scripts/hammer_print_r297_timer_integrated_test_only_matching_trigger_rehearsal_plan.sh"
    result = subprocess.run(
        ["bash", str(script)],
        cwd=REPO_ROOT,
        text=True,
    ...
    assert "R297 PRINT ONLY - timer-integrated test-only matching trigger rehearsal plan" in result.stdout
    assert "tiny-live-timer-integrated-test-only-matching-trigger-rehearsal" in result.stdout
...
def _build(tmp_path: Path, **kwargs) -> dict:
    ...
    defaults = {
        ...
        "reason": "R297 test-only timer-integrated rehearsal",
        "timer_observed_wait_certificate_packet": _wait_certificate(lane_key),
        "timer_health_packet": _timer_health(),
        "now": NOW,
    }
    defaults.update(kwargs)
    return build_tiny_live_timer_integrated_test_only_matching_trigger_rehearsal(**defaults)
...
def _assert_systemd_defaults(payload: dict) -> None:
    assert payload["installed_systemd_timer_mutated"] is False
    assert payload["installed_systemd_timer_fake_candidate_injection_enabled"] is False
    assert payload["normal_scheduler_default_simulation_enabled"] is False

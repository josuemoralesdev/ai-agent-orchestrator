...
from src.app.hammer_radar.operator.fundless_short_tiny_live_readiness_rehearsal import (
    ...
    FUNDLESS_SHORT_READINESS_REHEARSAL_READY,
    FUNDLESS_SHORT_READINESS_REHEARSAL_RECORDED,
    FUNDLESS_SHORT_READINESS_REHEARSAL_REJECTED,
    LEDGER_FILENAME,
    NOT_READY_FUNDING_AND_EVIDENCE_BLOCKED,
    build_fundless_short_tiny_live_readiness_rehearsal,
    load_fundless_readiness_rehearsal_records,
)
...
def test_preview_writes_no_record(tmp_path: Path) -> None:
    payload = build_fundless_short_tiny_live_readiness_rehearsal(
    ...
    assert payload["status"] == FUNDLESS_SHORT_READINESS_REHEARSAL_READY
    assert payload["rehearsal_recorded"] is False
    assert payload["rehearsal_id"] is None
    assert payload["record_rehearsal_requested"] is False
...
def test_wrong_confirmation_rejects_record(tmp_path: Path) -> None:
    payload = build_fundless_short_tiny_live_readiness_rehearsal(
        ...
        confirm_fundless_short_rehearsal="wrong",
        now=NOW,
    )

    assert payload["status"] == FUNDLESS_SHORT_READINESS_REHEARSAL_REJECTED
    assert payload["confirmation_valid"] is False
    assert payload["rehearsal_recorded"] is False
    assert load_fundless_readiness_rehearsal_records(log_dir=tmp_path / "logs", limit=0) == []


def test_exact_confirmation_records_rehearsal_only(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path / "lane_controls.json")
    ...
    payload = build_fundless_short_tiny_live_readiness_rehearsal(
        ...
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        record_rehearsal=True,
        confirm_fundless_short_rehearsal=CONFIRM_FUNDLESS_SHORT_REHEARSAL_RECORDING_PHRASE,
        now=NOW,
    )
    records = load_fundless_readiness_rehearsal_records(log_dir=tmp_path / "logs", limit=0)

    assert payload["status"] == FUNDLESS_SHORT_READINESS_REHEARSAL_RECORDED
    assert payload["confirmation_valid"] is True
    assert payload["rehearsal_recorded"] is True
    assert len(records) == 1
    assert records[0]["event_type"] == "FUNDLESS_SHORT_TINY_LIVE_READINESS_REHEARSAL"
...
def test_fresh_evidence_below_10_blocks_readiness(tmp_path: Path) -> None:
    log_dir = tmp_path / "logs"
    ...
    assert payload["evidence_gate"]["fresh_capture_count"] == 1
    assert payload["evidence_gate"]["required_fresh_capture_count"] == 10
    assert payload["evidence_gate"]["fresh_evidence_threshold_met"] is False
    assert payload["readiness"] == NOT_READY_FUNDING_AND_EVIDENCE_BLOCKED
    assert "fresh short capture sample below 10" in payload["blockers"]


def test_funding_unknown_blocks_readiness(tmp_path: Path) -> None:
    payload = build_fundless_short_tiny_live_readiness_rehearsal(
        log_dir=tmp_path / "logs",
        config_path=_write_config(tmp_path / "lane_controls.json"),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
    ...
    assert payload["funding_gate"]["account_funded"] is None
    assert payload["funding_gate"]["funding_status"] == "UNKNOWN_NOT_CHECKED"
    assert payload["funding_gate"]["balance_source"] == "not_checked"
    assert payload["funding_gate"]["funding_ready"] is False
    assert "account funding not verified" in payload["blockers"]


def test_non_executable_dry_run_intent_preview_is_not_executable(tmp_path: Path) -> None:
    payload = build_fundless_short_tiny_live_readiness_rehearsal(
        log_dir=tmp_path / "logs",
        config_path=_write_config(tmp_path / "lane_controls.json"),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
    ...
    preview = payload["non_executable_dry_run_intent_preview"]

    assert preview["would_build_order_payload"] is False
    assert preview["would_submit_order"] is False
    assert preview["would_call_binance"] is False
    assert preview["intent_type"] == "SHORT_TINY_LIVE_REHEARSAL_ONLY"
    assert preview["side"] == "SELL"
    assert preview["notional_usdt"] is None
    assert preview["executable"] is False


def test_no_order_payload_is_created_and_safety_flags_clean(tmp_path: Path) -> None:
    payload = build_fundless_short_tiny_live_readiness_rehearsal(
        log_dir=tmp_path / "logs",
        config_path=_write_config(tmp_path / "lane_controls.json"),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
...
def test_no_lane_mode_apply_or_live_commands_emitted(tmp_path: Path) -> None:
    ...
    assert "funds-dependent execution" in payload["do_not_run_yet"]
...
def test_cli_exists_and_preview_returns_expected_shape(tmp_path: Path) -> None:
    ...
    assert payload["target_family"]["lane_key"] == LANE_8M_SHORT
    assert "evidence_gate" in payload
    assert "funding_gate" in payload
    assert "fundless-short-tiny-live-readiness-rehearsal" in help_result.stdout

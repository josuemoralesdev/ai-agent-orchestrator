...
from src.app.hammer_radar.operator.short_risk_contract_apply_review import (
    APPLY_REVIEW_BLOCKED_BY_MULTIPLE_GATES,
    CONFIRM_SHORT_RISK_CONTRACT_APPLY_REVIEW_RECORDING_PHRASE,
    LEDGER_FILENAME,
    SHORT_RISK_CONTRACT_APPLY_REVIEW_BLOCKED,
    SHORT_RISK_CONTRACT_APPLY_REVIEW_RECORDED,
    SHORT_RISK_CONTRACT_APPLY_REVIEW_REJECTED,
...
def test_preview_writes_no_config(tmp_path: Path) -> None:
    ...
    payload = build_short_risk_contract_apply_review(
    ...
    assert payload["status"] == SHORT_RISK_CONTRACT_APPLY_REVIEW_BLOCKED
...
def test_preview_writes_no_review_record(tmp_path: Path) -> None:
    payload = build_short_risk_contract_apply_review(
        log_dir=tmp_path / "logs",
        config_path=_write_config(tmp_path / "lane_controls.json"),
    ...
    assert payload["review_recorded"] is False
    assert payload["review_id"] is None
    assert payload["record_review_requested"] is False
...
def test_wrong_confirmation_rejects_record(tmp_path: Path) -> None:
    payload = build_short_risk_contract_apply_review(
        ...
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
        record_review=True,
        confirm_short_risk_contract_apply_review="wrong",
        now=NOW,
    )

    assert payload["status"] == SHORT_RISK_CONTRACT_APPLY_REVIEW_REJECTED
    assert payload["confirmation_valid"] is False
    assert payload["review_recorded"] is False
    assert load_short_risk_contract_apply_review_records(log_dir=tmp_path / "logs", limit=0) == []
...
def test_exact_confirmation_records_review_only(tmp_path: Path) -> None:
    ...
    records = load_short_risk_contract_apply_review_records(log_dir=tmp_path / "logs", limit=0)

    assert payload["status"] == SHORT_RISK_CONTRACT_APPLY_REVIEW_RECORDED
    assert payload["confirmation_valid"] is True
    assert payload["review_recorded"] is True
    assert len(records) == 1
    assert records[0]["event_type"] == "SHORT_RISK_CONTRACT_APPLY_REVIEW"
    assert config_path.read_text(encoding="utf-8") == before_config
    assert risk_path.read_text(encoding="utf-8") == before_risk
...
def test_target_lane_default_is_8m_short_and_remains_paper(tmp_path: Path) -> None:
    ...
    assert payload["apply_readiness_matrix"]["lane_mode"]["satisfied"] is True


def test_missing_gates_block_apply_review(tmp_path: Path) -> None:
    payload = build_short_risk_contract_apply_review(
        log_dir=tmp_path / "logs",
        config_path=_write_config(tmp_path / "lane_controls.json"),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
    ...
    matrix = payload["apply_readiness_matrix"]
    assert matrix["fresh_evidence"]["satisfied"] is False
    assert matrix["r158_promotion_readiness"]["satisfied"] is False
    assert matrix["funding"]["current"] == "UNKNOWN_NOT_CHECKED"
    assert matrix["funding"]["satisfied"] is False
    assert matrix["operator_approval"]["current"] == "not provided"
    assert matrix["operator_approval"]["satisfied"] is False
    assert matrix["config_write_authorization"]["current"] == "not authorized"
    assert matrix["config_write_authorization"]["satisfied"] is False
    assert payload["readiness"] == APPLY_REVIEW_BLOCKED_BY_MULTIPLE_GATES
    assert "fresh captures below threshold" in payload["blockers"]
    assert "funding not verified" in payload["blockers"]
    assert "operator approval missing" in payload["blockers"]


def test_future_config_patch_preview_is_preview_only(tmp_path: Path) -> None:
    payload = build_short_risk_contract_apply_review(
        log_dir=tmp_path / "logs",
        config_path=_write_config(tmp_path / "lane_controls.json"),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
...
def test_safe_commands_have_no_live_lane_apply_or_order_commands(tmp_path: Path) -> None:
    payload = build_short_risk_contract_apply_review(
        log_dir=tmp_path / "logs",
        config_path=_write_config(tmp_path / "lane_controls.json"),
        risk_contract_config_path=_write_risk_contract_config(tmp_path / "tiny_live_risk_contracts.json"),
    ...
    assert "short-risk-contract-draft-preview" in joined
    assert "short-risk-contract-apply-review" in joined
...
def test_cli_exists_and_preview_returns_expected_shape(tmp_path: Path) -> None:
    ...
    assert payload["target_family"]["lane_key"] == LANE_8M_SHORT
    assert "existing_contract_state" in payload
    assert "latest_draft_summary" in payload
    assert "apply_readiness_matrix" in payload
    assert "future_config_patch_preview" in payload
    assert "short-risk-contract-apply-review" in help_result.stdout
